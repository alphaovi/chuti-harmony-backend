...
const updateSingleFeatureProjectIntoDB = async(id: string, payload:Partial<TFeatureProjects>) => {
    const objectId = new mongoose.Types.ObjectId(id);

    const result = await FeatureProjectsModel.findOneAndUpdate({_id: objectId}, payload, {new: true})
